...
inline int countSetBits(int x) { int count = 0;while (x){x &= (x-1);count++;} return count; }
...
int main()
{
  ...
  while(T--){
	 ...
	 sort(all(s));
	 string :: iterator r= lower_bound(all(s),p[0]);
	 string :: iterator u= upper_bound(all(s),p[0]);
	 if(r!=u){
	 	bool instinct;
	 	for(int i=1;i<p.length();i++){
	 		if(p[i]<p[0]){
			 	...
			 	instinct=false;break;
			 }
		 }
		 if(instinct){
		 	s.insert(r-s.begin(),p);
		 }
		 else{
		 	s.insert(u-s.begin(),p);
		 }
		 cout<<s<<"\n";
		 continue;
	 }
